...
public class client implements ActionListener{
    ...
    client() {
        ...
        ImageIcon i4 = new ImageIcon(ClassLoader.getSystemResource("icons/profile2.jpg")); 
        Image i5 = i4.getImage().getScaledInstance(50, 50 , Image.SCALE_DEFAULT);
        ImageIcon i6 = new ImageIcon(i5);
        JLabel profile2 = new JLabel(i6);
        profile2.setBounds(40,10,50,50);
        p1.add(profile2);
        ...
        JLabel name = new JLabel("Nainika");
        name.setBounds(110,15,100,18);
        name.setForeground(Color.white);
        name.setFont(new Font("SAN_SERIF",Font.BOLD,16));
        p1.add(name); 

        JLabel status = new JLabel("Good to go");
        status.setBounds(110,35,100,18);
        status.setForeground(Color.white);
        status.setFont(new Font("SAN_SERIF",Font.PLAIN,16));
        p1.add(status); 
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        JPanel pr = formatLabel(result);
 
        area.setLayout(new BorderLayout());

        JPanel right = new JPanel(new BorderLayout());
        right.add(pr,BorderLayout.LINE_END);
        vertical.add(right);
